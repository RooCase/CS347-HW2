...
class Evaluation(object):
    ...
    def measure_place(self):
        ...
        sizes = [-1, 0, 1, 5, 15, 99, 100, 101, 1000, 12500] # bin sizes we will test
        total = 0 # total time to be returned
        for size in sizes:
            start_time = time.time() # testing each placement size
            response = bin_size.place_item(test_problem, size)
            end_time = time.time()
    ...
    def measure_newend(self):
        ...
        for t_time in times:
            ...
            end_time = time.time()
            self.logger.info(f"Execution time for new_problem {t_time} times: {end_time-start_time} seconds, response: {response}")
